...
- **game-changing-features** (`.agents/skills/game-changing-features/`): MUST BE ACTIVATED when the user asks about product strategy, 10x improvements, what to build next, high-impact features, or game-changing opportunities. This skill writes ALL output to .agents/docs/ai/<area>/10x/session-N.md as a formal artifact. It NEVER outputs strategy to chat. Activate via run_command on its init and verify scripts.
